 ...
 */
export default function PrepListPanel({ groupId, venueId, canEdit, showToast }) {
  ...
  const add = async () => {
    if (!text.trim()) return;
    try {
      await addDoc(col(), { text: text.trim(), qty: qty.trim(), note: note.trim(), done: false, order: items.length, createdAt: serverTimestamp() });
      setText(""); setQty(""); setNote("");
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      <div className="card-head">
        <div style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }} onClick={() => setOpen((o) => !o)}>
          <span className="card-title">🥬 Prep list <span style={{ color: "var(--gray)", fontWeight: 400 }}>({items.length - doneCount} to do)</span></span>
          <span className="card-sub">carries over — not reset daily</span>
        </div>
      ...
      {open && (
        <>
          {items.map((it) => (
            <div key={it.id} className="checklist-item" style={{ alignItems: "flex-start" }}>
          ...
          {canEdit && (
            <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap" }}>
              ...
              <input className="form-input" style={{ width: 90 }} value={qty} onChange={(e) => setQty(e.target.value)} placeholder="Qty" />
              <input className="form-input" style={{ flex: 1, minWidth: 120 }} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Note (optional)" onKeyDown={(e) => e.key === "Enter" && add()} />
