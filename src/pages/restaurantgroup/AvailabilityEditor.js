...
// availabilityWindow (modGroupKind/staffSeesAll convention; this copy lives in-component).
...
// CROSS-REPO SHARED HELPER — must stay BYTE-IDENTICAL to Ops lib/availabilityModel's
// isDayLocked (same twin convention as availabilityWindow above; this copy lives
// in-component).
// THE FRIDAY LOCK (owner-confirmed): a week's availability closes at 11:59pm on the
// Friday BEFORE that week starts. Over the 21-day window (index 0 = the anchor Monday):
//   0-6   current week → ALWAYS locked (its Friday passed last week)
//   7-13  next week    → locked once `now` is past Friday 23:59:59.999 of the CURRENT week
//   14-20 third week   → never locked (its deadline is still a week+ away)
// `monday` is the SAME anchor the caller fed availabilityWindow (Ops mondayOf(0) / Admin
// mondayFromWeekKey(currentWeekKey())); the cutoff is built from its LOCAL parts — never
// by parsing a date string (bare new Date("YYYY-MM-DD") is UTC midnight). Passing the
// window's anchor also keeps a STALE session honest: a window mounted last week evaluates
// against ITS OWN Friday, so its "next week" indices lock the moment that Friday passes.
// DEVICE-LOCAL TIME: neither app pins a client timezone, so a mis-set or overseas device
// gets a displaced cutoff — and nothing server-side enforces the lock (the availability
// rule is flat member-write). This is a UX boundary, not a security one (CLAUDE.md RULE 2).
// Tier-blind by design: the CALLER exempts can("availability","edit") holders.
const isDayLocked = (dayIndex, monday, now = new Date()) => {
  if (dayIndex <= 6) return true;
  if (dayIndex >= 14) return false;
  const cutoff = new Date(monday.getFullYear(), monday.getMonth(), monday.getDate() + 4, 23, 59, 59, 999);
...
export default function AvailabilityEditor({ staff, onSaved }) {
  ...
  const days = useMemo(() => availabilityWindow(anchorMonday), [anchorMonday]);
  ...
  const save = async () => {
    ...
    if (!lockExempt && toWrite.some((d) => isDayLocked(days.indexOf(d), anchorMonday, new Date()))) {
  ...
  return (
    <>
      ...
      <div className="card" style={{ marginBottom: 12, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 700 }}>{isSelf ? "My availability — next 3 weeks" : `${staff.displayName || staff.name || "Staff"} — availability, next 3 weeks`}</div>
      ...
      {days.map((d, i) => {
        const v = val(d);
        const ex = existing(d);
        // FRIDAY LOCK — read-only row, no controls; the save guard is the backstop
        const locked = !lockExempt && isDayLocked(i, anchorMonday, new Date());
        if (locked) {
          const posted = ex
            ? (ex.available ? (ex.allDay !== false ? "Available: all day" : `Available: ${(ex.windows || []).map((w) => `${w.start}–${w.end}`).join(", ")}`) : "Unavailable")
            : "Not set";
          return (
            <div key={d} className="card" style={{ marginBottom: 8, padding: "10px 14px", opacity: 0.75 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                <span style={{ marginLeft: "auto", fontSize: 10, fontWeight: 700, color: "#9ca3af" }}>Availability for this week has closed</span>
              </div>
            </div>
          );
        }
