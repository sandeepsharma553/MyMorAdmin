...
import {
  ...
  query, getDoc, Timestamp, writeBatch,
...
export default function EventPage({ navbarHeight }) {
    ...
    (async () => {
      try {
        ...
        snap.docs.forEach((d) => {
          ...
          const key = b.eventId || b.eventDocId || "__unknown__";
  ...
  const getList = async () => {
    ...
    try {
      ...
      docs.sort(
        (a, b) =>
          (toMillis(a.startDateTime) ?? 0) -
          (toMillis(b.startDateTime) ?? 0)
      );
      setList(docs);
    } catch (e) {
      console.error(e);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const hasPoster =
        (form.posters?.length || 0) > 0 ||
        (form.posterFiles?.length || 0) > 0;
      ...
      if (form.priceType === "Paid") {
        ...
        if (invalid) {
          return toast.error("Please enter ticket name for all tickets.");
        }
      }
      ...
      if (form.posterFiles?.length) {
        const uploads = form.posterFiles.map(async (file) => {
          const path = uniquePath(
            `public_event_posters/${emp.id}/${form.eventName || "publicevents"}`,
            file
          );
  ...
  const renumberPinned = async () => {
    ...
    pinned.forEach((ev, i) => {
      ...
      if (ev.pinnedOrder !== order) {
        batch.update(doc(db, "publicevents", ev.id), { pinnedOrder: order });
      }
    });
  ...
  const movePin = async (item, dir) => {
    ...
    const batch = writeBatch(db);
    batch.update(doc(db, "publicevents", a.id), {
      pinnedOrder: b.pinnedOrder,
    });
    batch.update(doc(db, "publicevents", b.id), {
  ...
  const applyPinOrder = async (item, newOrderRaw) => {
    ...
    sequence.forEach((ev, i) =>
      batch.update(doc(db, "publicevents", ev.id), { pinnedOrder: i + 1 })
    );
                ...
                sorted.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                      {(() => {
                        const arr =
                          bookingsByEvent[item.id] ||
                          bookingsByEvent[item.eventName] ||
                          [];
                                ...
                                {[
                                  ...
                                  <label key={field}>
                                    <input
                                      ...
                                      onChange={() =>
                                        updateTicketFieldCheckbox(index, field)
                                      }
                                      className="mr-1"
                                    />
                                    {field}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  {(() => {
                    ...
                    const start =
                      (bookingModal.page - 1) * bookingModal.pageSize;
