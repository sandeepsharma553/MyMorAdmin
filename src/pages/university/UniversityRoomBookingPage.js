// src/pages/UniversityRoomBookingPage.jsx
...
export default function UniversityRoomBookingPage({ navbarHeight }) {
  ...
  const addTypesFromString = (src) => {
    ...
    setForm((prev) => ({
      ...prev,
      roomtypes: Array.from(new Set([...(prev.roomtypes || []), ...parts])),
    }));
    setRoomTypeInput("");
  };
  ...
  const removeType = (t) =>
    setForm((prev) => ({
      ...prev,
      roomtypes: (prev.roomtypes || []).filter((x) => x !== t),
    }));
  ...
  }, [roomFilter, roomTypeFilter, universityId]);

  useEffect(() => {
    setCurrentPage(1);
  }, [filters, sortConfig, roomFilter, roomTypeFilter, range, timeRange]);
  ...
  const getList = async () => {
    ...
    try {
      ...
      userSnap.forEach((d) => {
        const u = d.data();
        userMap[u.uid] = {
          username: u.username || u.UserName || u.USERNAME || "Unknown",
      ...
      const uniqueRooms = Array.from(
        new Set(BookingType.map((b) => b.roomname))
      ).filter(Boolean);

      const allTypesLocal = Array.from(
        new Set(BookingType.flatMap((rt) => rt.roomtypes || []))
      );
      ...
      const roomsForType = new Set(
        roomTypeFilter === "All"
          ...
          : BookingType
              .filter((x) => (x.roomtypes || []).includes(roomTypeFilter))
              .map((x) => x.roomname)
      );
      ...
      const roomFiltered =
        roomFilter === "All"
          ? roomFiltered0
          : roomFiltered0.filter((b) => b.roomname === roomFilter);
  ...
  const handleAdd = async (e) => {
    ...
    for (const k of DAY_KEYS) {
      ...
      if (day?.open) {
        ...
        const ed = hmToMinutes(day.end);
        if (s == null || ed == null) {
          toast.warning(`${DAY_LABEL[k]}: start/end must be HH:mm`);
          return;
        }
        if (s === ed) {
          toast.warning(`${DAY_LABEL[k]}: start and end cannot be same`);
          return;
        }
      }
    }
    ...
    try {
      if (editingData) {
        ...
        if (!docSnap.exists()) {
          toast.warning("BookingRoomType does not exist!");
          return;
        }
  ...
  const deleteSelectedBookings = async () => {
    ...
    if (
      !window.confirm(
        `Delete ${selectedBookingIds.size} booking(s)? This cannot be undone.`
      )
    )
      return;
  ...
  const deleteSelectedRoomTypes = async () => {
    ...
    if (
      !window.confirm(
        `Delete ${selectedTypeIds.size} room type(s)? This cannot be undone.`
      )
    )
      return;
  ...
  const withinDateHeaderRange = (row) => {
    ...
    if (!s || !e) {
      ...
      return (
        d0 >= toLocalDateOnly(r.startDate) && d0 <= toLocalDateOnly(r.endDate)
      );
    }

    const s0 = toLocalDateOnly(s);
    const e0 = toLocalDateOnly(e);
    const rs = toLocalDateOnly(r.startDate);
    const re = toLocalDateOnly(r.endDate);
  ...
  const withinTimeHeaderRange = (row) => {
    ...
    if (filterStartM <= filterEndM) {
      ...
      const a = rowStartM < 1440 && rowEndM > filterStartM;
      const b = rowStartM < filterEndM && rowEndM > 0;
      return a || b;
    }
  };
  ...
  const sortedData = [...filteredData].sort((a, b) => {
    ...
    if (sortConfig.key === "time") {
      const ta = a.displayTime || a.time || "";
      const tb = b.displayTime || b.time || "";
  ...
  const allBookingsPageSelected =
    bookingPageIds.length > 0 &&
    bookingPageIds.every((id) => selectedBookingIds.has(id));
  const someBookingsPageSelected = bookingPageIds.some((id) =>
    selectedBookingIds.has(id)
  );

  const formattedRange =
    dateActive && range[0]?.startDate && range[0]?.endDate
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 flex-wrap mb-2">
        ...
        >
          {["All", ...rooms.filter((v, i, a) => a.indexOf(v) === i)].map(
            (room) => (
                ...
                <th className="px-6 py-3">
                  <input
                    ...
                    ref={(el) => {
                      if (el) {
                        el.indeterminate =
                          !allBookingsPageSelected && someBookingsPageSelected;
                      }
                    }}
                    onChange={(e) => {
                      setSelectedBookingIds((prev) => {
                        ...
                        if (e.target.checked)
                          bookingPageIds.forEach((id) => next.add(id));
              ...
              <tr className="border-t border-gray-200">
                <th className="px-6 pb-3">
                  <input
                    ...
                    onChange={(e) =>
                      setFilterDebounced("username", e.target.value)
                    }
                  />
                </th>
                <th className="px-6 pb-3">
                  <input
                    ...
                    onChange={(e) =>
                      setFilterDebounced("email", e.target.value)
                    }
                  />
                </th>
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-semibold ${
                          item.status === "Rejected"
                            ...
                            : item.status === "Approved" || item.status === "Booked"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 rounded-lg shadow-lg">
            <h2 className="text-xl font-bold mb-4">
              {editingData ? "Edit Room Type" : "Add Room Type"}
            </h2>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        addTypesFromString(roomTypeInput);
                      }
                    }}
                  />
                  <button
                  ...
                  <div className="col-span-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          maxDurationMins: Number(e.target.value),
                        })
                      }
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Keep ≤ 120 for 2-hour max.
                    </p>
                  </div>
                  <div>
                ...
                <div className="mt-2">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    >
                      Fill {form.startTime || "--:--"}–{form.endTime || "--:--"}
                    </button>
                  </div>
                      ...
                      <div key={k} className="grid grid-cols-6 items-center gap-2">
                        ...
                        <div className="col-span-1 flex items-center gap-2">
                          ...
                          <span className="text-sm">
                            {form.weeklyHours[k]?.open ? "Open" : "Closed"}
                          </span>
                        </div>
                        <div className="col-span-1">
                          <input
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Leave a day closed to disable bookings that day.
                  </p>
                </div>
              </div>
            ...
            <form onSubmit={() => {}} className="space-y-4">
              <div className="space-y-4">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setBookingForm({ ...bookingForm, roomname: e.target.value })
                  }
                  required
                >
                  ...
                  {bookingTypeList.map((rt) => (
                    <option key={rt.id} value={rt.roomname}>
                      {rt.roomname}
                    </option>
                  ))}
                </select>
              ...
              <div className="flex justify-end mt-6 space-x-3">
                ...
                <button
                  ...
                  type="button"
                  onClick={() => toast.info("Hook this to handleAddBooking if needed")}
                >
                  Save Booking
                </button>
              </div>
          ...
          <div className="bg-white p-6 rounded-lg w-[95%] max-w-6xl shadow-lg max-h-[90vh] overflow-y-auto">
            ...
            {selectedTypeIds.size > 0 && (
              <div className="mb-2 flex items-center gap-3">
                ...
                <button
                  onClick={() =>
                    setSelectedTypeIds(new Set(bookingTypeList.map((r) => r.id)))
                  }
                  className="px-3 py-1.5 bg-gray-200 rounded text-sm"
                >
                    ...
                    <th className="px-6 py-3">
                      <input
                        ...
                        checked={
                          ...
                          bookingTypeList.every((x) => selectedTypeIds.has(x.id))
                        }
                        ref={(el) => {
                          if (!el) return;
                          const all =
                            bookingTypeList.length > 0 &&
                            bookingTypeList.every((x) => selectedTypeIds.has(x.id));
                          const some = bookingTypeList.some((x) =>
                            selectedTypeIds.has(x.id)
                          );
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setForm({
                              ...
                              weeklyHours:
                                roomData.weeklyHours || defaultWeeklyHours(),
                              roomtypes: Array.isArray(roomData.roomtypes)
                                ? roomData.roomtypes
                                : [],
                            });
                            setModalOpen(true);
            ...
            {confirmDeleteOpen && (
              <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
                <div className="bg-white p-6 rounded-lg w-80 shadow-lg">
                  ...
                  <p className="mb-4">
                    Delete <strong>{deleteData?.roomname}</strong>?
                  </p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-80 shadow-lg">
            ...
            <p className="mb-4">
              ...
              <strong>{editingData?.roomname}</strong>?
            </p>
            <div className="flex justify-end space-x-3">
              <button
                onClick={() => {
