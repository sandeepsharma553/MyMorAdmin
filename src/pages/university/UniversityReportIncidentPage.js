// src/pages/UniversityReportIncidentPage.jsx
...
export default function UniversityReportIncidentPage(props) {
    ...
    try {
      ...
      if (!isAdmin) {
        ...
        rows = rows.filter((r) => {
          ...
          const matchEmail =
            (r.assignedToEmail &&
              String(r.assignedToEmail).toLowerCase() === me) ||
            emails.includes(me);
  ...
  const openAssign = (row) => {
    ...
    const draft = Array.isArray(row.assignedToEmails)
      ...
      : row.assignedToEmail
      ? [String(row.assignedToEmail).toLowerCase()]
      : [];
  ...
  const saveAssignment = async () => {
    ...
    try {
      ...
      const candidate =
        extra && isValidEmail(extra) ? [...assignEmails, extra] : assignEmails;
      const emails = Array.from(new Set(candidate.map(normalizeEmail))).filter(
        isValidEmail
      );
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    switch (sortConfig.key) {
      ...
      case "date": {
        const ad =
          a.datetime?.seconds !== undefined
            ? a.datetime.seconds * 1000
            : Date.parse(a.datetime || 0) || 0;
        const bd =
          b.datetime?.seconds !== undefined
            ? b.datetime.seconds * 1000
            : Date.parse(b.datetime || 0) || 0;
        return (ad - bd) * dir;
      }
      case "status":
                  ...
                  return (
                    <tr key={item.id}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {item.isreport === false ? item.username : "Anonymous"}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {item.incidenttype}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {Array.isArray(item.assignedToEmails) &&
                        item.assignedToEmails.length > 0 ? (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 rounded-lg shadow-lg">
            <h2 className="text-xl font-bold mb-4">
              {editingData ? "Edit Incident" : "Add Incident"}
            </h2>
          ...
          <div className="bg-white w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 rounded-lg shadow-lg">
            ...
            <div ref={viewPrintRef} className="space-y-3">
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                ...
                <span>
                  {Array.isArray(viewData?.assignedToEmails) &&
                  viewData.assignedToEmails.length > 0
            ...
            <div className="space-y-3">
              ...
              <div className="border rounded p-2 min-h-[44px] flex flex-wrap gap-2">
                {assignEmails.map((em) => (
                  <span
                    key={em}
                    className="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 border rounded"
                  >
            ...
            <div className="flex justify-between items-center gap-3 mt-5">
              <div className="text-xs text-gray-500">
                {assignEmails.length > 0
                  ? `${assignEmails.length} assignee(s)`
                  : "No assignees yet"}
              </div>
