...
export default function UniversityGuestLogPage({ navbarHeight }) {
  ...
  const loadLogs = async () => {
    setLoading(true);
    try {
      const snap = await getDocs(query(collection(db, "university", universityId, "guestLogs"), orderBy("createdAt", "desc")));
  ...
  const loadPolicy = async () => {
    try {
      const snap = await getDoc(doc(db, "university", universityId, "guestPolicy", "config"));
  ...
  const handleFlag = async () => {
    if (!flagModal) return;
    try {
      await updateDoc(doc(db, "university", universityId, "guestLogs", flagModal.id), {
  ...
  const savePolicy = async (e) => {
    ...
    try {
      await setDoc(doc(db, "university", universityId, "guestPolicy", "config"), {
        ...policy,
        updatedAt: serverTimestamp(),
      });
