...
export default function UniversityAnnouncementPage(props) {
  ...
  const timeFiltered = list.filter(
    (item) => classifyByDate(item.date) === timeFilter
  );
  ...
  const uploadAllPosters = async (files) => {
    ...
    for (let i = 0; i < files.length; i++) {
      const f = files[i];
      const path = `university_announcements_posters/${Date.now()}_${i}_${f.name}`;
  ...
  const maxPinnedOrder = () =>
    Math.max(
      0,
      ...list.filter((x) => x.isPinned).map((x) => Number(x.pinnedOrder) || 0)
    );
  ...
  const handleChange = (e) => {
    ...
    if (type === "checkbox") {
      if (name === "allowMulti") {
        setForm((prev) => ({
          ...prev,
          pollData: { ...prev.pollData, allowMulti: checked },
        }));
      } else if (name === "allowaddoption") {
        setForm((prev) => ({
          ...prev,
          pollData: { ...prev.pollData, allowaddoption: checked },
        }));
    ...
    if (name === "question") {
      setForm((prev) => ({
        ...prev,
        pollData: { ...prev.pollData, question: value },
      }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error("Failed to save university announcement");
    } finally {
      setIsLoading(false);
      setModalOpen(false);
      setEditing(null);
    ...
    setForm((prev) => ({
      ...
      pollData: {
        ...
        options: {
          ...
          [key]: { ...prev.pollData.options[key], text: newText },
        },
      },
    }));
  };
  ...
  const removeOption = (key) => {
    setForm((prev) => {
      ...
      return {
        ...prev,
        pollData: { ...prev.pollData, options: updated },
      };
    });
  };
    ...
    return (
      <main className="flex-1 p-6 bg-gray-100 overflow-auto">
        <div className="bg-white rounded-xl shadow p-10 text-center text-gray-500">
          No university assigned to current employee.
        </div>
      </main>
    );
  }
                ...
                <tr className="border-t border-gray-200">
                  ...
                  <th className="px-6 pb-3">
                    <input
                      ...
                      onChange={(e) =>
                        setFilterDebounced("desc", e.target.value)
                      }
                    />
                  </th>
                ...
                {paginatedData.length === 0 ? (
                  <tr>
                    <td colSpan="7" className="px-6 py-4 text-center text-gray-500">
                      No university announcements found.
                    </td>
                  </tr>
                ) : (
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className={`text-lg leading-none ${
                              item.isPinned ? "text-yellow-500" : "text-gray-400"
                            } hover:opacity-80`}
                            aria-label={
                              item.isPinned
                                ? "Unpin university announcement"
                                : "Pin university announcement"
                            }
                          >
                            {item.isPinned ? "★" : "☆"}
                          </button>
                          ...
                          {item.isPinned && (
                            <div className="flex items-center gap-1">
                              <input
                                ...
                                onChange={(e) =>
                                  setPinnedOrder(item, e.target.value)
                                }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 rounded-lg shadow-lg">
            <h2 className="text-xl font-bold mb-4">
              {editingData ? "Edit University Announcement" : "Add University Announcement"}
            </h2>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-80 shadow-lg">
            <h2 className="text-xl font-semibold mb-4 text-red-600">
              Delete University Announcement
            </h2>
            <p className="mb-4">
