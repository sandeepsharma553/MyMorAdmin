// src/pages/UniversityDealPage.jsx
...
export default function UniversityDealPage(props) {
  ...
  const filteredByTime = filteredBySearch.filter(
    (item) => classifyDeal(item) === timeFilter
  );
  ...
  const filteredByHeader = filteredByTime.filter((item) => {
    ...
    const discOK =
      ...
      String(item.discount || "")
        .toLowerCase()
        .includes(filters.discount.toLowerCase());

    const startS = fmt(item.startDateTime);
    const endS = fmt(item.endDateTime);

    const startOK =
      !filters.start || startS.toLowerCase().includes(filters.start.toLowerCase());
    const endOK =
      !filters.end || endS.toLowerCase().includes(filters.end.toLowerCase());
