...
export default function UniversityResourcesPage(props) {
          ...
          const images = await Promise.all(
            (row.images || []).map(async (it) => {
              if (isDocObj(it) && it.url) {
                return {
                  url: it.url,
                  name: it.name || inferFileNameFromUrl(it.url),
                };
              }
  ...
  const nudgeOrder = async (row, delta) =>
    setPinnedOrder(row, Number(row.pinnedOrder || 0) + delta);
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className={`text-lg leading-none ${
                            x.isPinned ? "text-yellow-500" : "text-gray-400"
                          } hover:opacity-80`}
                          aria-label={
                            x.isPinned ? "Unpin resource" : "Pin resource"
                          }
                        >
                          {x.isPinned ? "★" : "☆"}
                        </button>
                ...
                >
                      ...
                      <div key={i} className="flex gap-2 mb-1">
                        ...
                        {row.contacts.length > 1 && (
                          <button
                            type="button"
                            onClick={() =>
                              removeFieldItem(rowIdx, "contacts", i)
                            }
                          >
                            ✕
                          </button>
                    ...
                    {row.links.map((linkObj, i) => (
                      <div key={i} className="flex gap-2 mb-1">
                        ...
                        {row.links.length > 1 && (
                          <button
                            type="button"
                            onClick={() => removeFieldItem(rowIdx, "links", i)}
                          >
                            ✕
                          </button>
                        )}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="hidden"
                      ref={(el) => {
                        imageInputsRef.current[rowIdx] = el;
                      }}
                      onChange={(e) => {
                        const picked = Array.from(e.target.files || []).map(
                          (f) => ({ file: f, name: f.name })
                        );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-80 shadow-lg">
            ...
            <p className="mb-4">
              ...
              <strong>{deleteData?.title || "this resource"}</strong>?
            </p>
