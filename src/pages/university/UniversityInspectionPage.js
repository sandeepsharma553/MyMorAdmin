...
export default function UniversityInspectionPage({ navbarHeight }) {
  ...
  const loadInspections = async () => {
    setLoading(true);
    try {
      const snap = await getDocs(query(collection(db, "university", universityId, "inspections"), orderBy("submittedAt", "desc")));
  ...
  const loadTemplates = async () => {
    try {
      const snap = await getDocs(collection(db, "university", universityId, "inspectionTemplates"));
  ...
  const saveTemplate = async (e) => {
    ...
    try {
      ...
      if (editingTemplate) {
        await updateDoc(doc(db, "university", universityId, "inspectionTemplates", editingTemplate.id), payload);
        toast.success("Template updated");
      } else {
        await addDoc(collection(db, "university", universityId, "inspectionTemplates"), { ...payload, createdAt: serverTimestamp(), createdBy: user?.uid || "" });
  ...
  const deleteTemplate = async (id) => {
    ...
    try { await deleteDoc(doc(db, "university", universityId, "inspectionTemplates", id)); toast.success("Deleted"); loadTemplates(); }
  ...
  const saveAdminNote = async () => {
    ...
    try {
      await updateDoc(doc(db, "university", universityId, "inspections", notesModal.id), { adminNotes: adminNote.trim(), updatedAt: serverTimestamp() });
