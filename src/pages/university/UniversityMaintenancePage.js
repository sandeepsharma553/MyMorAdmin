...
export default function UniversityMaintenancePage(props) {
  ...
  const [assignFreeText, setAssignFreeText] = useState("");
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [filters, sortConfig, listDateRange]);

  useEffect(() => {
    if (!universityId || !authUser?.uid) return;
    const doReset = async () => {
      const refd = doc(
        db,
        "adminMenuState",
  ...
  const resolveEmployeesByEmails = async (emails) => {
    const results = [];
  ...
  const deleteSelected = async () => {
    ...
    if (
      !window.confirm(
        `Delete ${selectedIds.size} maintenance request(s)? This cannot be undone.`
      )
    )
      return;
    setIsLoading(true);
  ...
  const saveAssignment = async () => {
    ...
    if (assignFreeText.trim()) {
      emails = emails.concat(
        assignFreeText.split(/[,\s]+/).map(normEmail).filter(Boolean)
      );
    }
    ...
    try {
      ...
      const noteEntry = assignNote.trim()
        ...
        : null;
      const prevNotes = Array.isArray(current?.adminNotes)
        ? current.adminNotes
        : [];
  ...
  const filteredList = list.filter((r) => {
    ...
    const mtOK =
      ...
      maintenancetypeStr === filters.maintenancetype.toLowerCase();
    const issueOK =
      filters.issue === "All" || issueStr === filters.issue.toLowerCase();

    const mineFlag = isAdmin ? filters.mineOnly : true;
    const mineOK =
      !mineFlag ||
      getAssigneesFromRow(r).some(
        (a) => a.email === normEmail(myEmail) || (a.uid && a.uid === uid)
      );

    let rangeOK = true;
    if (listDateRange?.startDate && listDateRange?.endDate) {
      const itemDate =
        parseDateSafe(r.createdDate) || parseDateSafe(r.createdAt) || null;
      rangeOK = inRangeInclusive(
        itemDate,
  ...
  const sortedList = [...filteredList].sort((a, b) => {
    ...
    switch (sortConfig.key) {
      ...
      case "issue":
        return (a.problemcategory || "").localeCompare(
          b.problemcategory || ""
        ) * dir;
      case "location":
        return (a.roomno || "").localeCompare(b.roomno || "") * dir;
      case "maintenancetype":
        return (a.maintenancetype || "").localeCompare(
          b.maintenancetype || ""
        ) * dir;
      case "status":
      ...
      case "date": {
        const ad = (
          parseDateSafe(a.createdDate) ||
          parseDateSafe(a.createdAt) ||
          new Date(0)
        ).getTime();
        const bd = (
          parseDateSafe(b.createdDate) ||
      ...
      case "assigned":
        return (
          (getAssigneesFromRow(a)[0]?.email || "").localeCompare(
            getAssigneesFromRow(b)[0]?.email || ""
          ) * dir
        );
      default:
        return 0;
  ...
  return (
    ...
    >
      ...
      <div>(When notification appears, refresh page)</div>
              ...
              <tr className="border-t border-gray-200">
                ...
                <th className="px-6 pb-3">
                  <select
                    ...
                    value={filters.issue}
                    onChange={(e) =>
                      setFilters((p) => ({ ...p, issue: e.target.value }))
                    }
                  >
                    ...
                    >
                      {listDateRange?.startDate && listDateRange?.endDate
                        ? `${format(
                            listDateRange.startDate,
                            "MMM dd, yyyy"
                          )} – ${format(listDateRange.endDate, "MMM dd, yyyy")}`
                        : "Filter by date…"}
                    </button>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {getAssigneesFromRow(item).length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          ...
                          {Array.isArray(item.adminNotes) &&
                            item.adminNotes.length > 0 && (
                              <span className="text-xs text-gray-500 ml-1">
                                ({item.adminNotes.length} note
                                {item.adminNotes.length > 1 ? "s" : ""})
                                {getUnseenNotesCount(item) > 0 && (
                                  <span
                                    title={`${getUnseenNotesCount(
                                      item
                                    )} new note(s)`}
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              <div className="space-y-4">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm({ ...form, problemcategory: e.target.value })
                  }
                  required
                >
                ...
                <div>
                  <label className="flex items-center gap-2">
                    ...
                    />
                    <span>
                      I agree to allow a staff member to enter my room to complete
                      the requested maintenance work, even if I am not present at
                      the time.
                    </span>
                  </label>
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-80 shadow-lg">
            ...
            <p className="mb-4">
              ...
              <strong>{deleteData?.username}</strong>'s maintenance request?
            </p>
          ...
          <div className="bg-white w-full max-w-lg max-h-[90vh] overflow-y-auto p-6 rounded-lg shadow-lg">
            ...
            <div ref={viewPrintRef} className="space-y-3">
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                ...
                <span className="font-medium">Description:</span><span>{viewData?.description}</span>
                <span className="font-medium">Cause:</span><span>{viewData?.cause || "—"}</span>
                <span className="font-medium">Comments:</span><span>{viewData?.comments || "—"}</span>
              ...
              {Array.isArray(viewData?.adminNotes) &&
                viewData.adminNotes.length > 0 && (
                  <div className="mt-3">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-6xl max-h-[90vh] overflow-y-auto p-6 rounded-lg shadow-lg">
            <div ref={listPrintRef} className="sheet">
              <div className="h1 text-xl font-bold mb-4 text-center">
                All University Maintenance Requests
              </div>
                  ...
                  {list.map((item, idx) => {
                    ...
                    return (
                      <tr key={idx} className="odd:bg-white even:bg-gray-50">
                        ...
                        <td className="border p-2">
                          {getAssigneesFromRow(item)[0]?.email || "—"}
                        </td>
          ...
          <div className="bg-white w-full max-w-md p-6 rounded-lg shadow-lg">
            ...
            <div className="space-y-3">
              <div className="text-sm text-gray-600">
                <div>
                  <span className="font-medium">Request:</span> {assignTarget?.id}
                </div>
                <div>
              ...
              {adminEmails.length > 0 && (
                <div>
                  ...
                  >
                    {adminEmails.map((em) => (
                      <option key={em} value={normEmail(em)}>
                        {em}
                      </option>
                    ))}
                  </select>
              ...
              <div>
                <label className="block text-sm font-medium mb-1">
                  Add emails (comma or space separated)
                </label>
                <input
                  type="text"
                  className="w-full border border-gray-300 p-2 rounded"
              ...
              <div className="flex flex-wrap gap-1">
                {assignEmails.map((em) => (
                  <span
                    key={em}
                    className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-gray-100 border text-gray-700"
                  >
                    {em}
                    <button
                      type="button"
                      ...
                      onClick={() =>
                        setAssignEmails(assignEmails.filter((x) => x !== em))
                      }
                      title="Remove"
                    >
                      ✕
