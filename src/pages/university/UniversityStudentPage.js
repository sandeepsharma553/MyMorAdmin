...
export default function UniversityStudentPage(props) {
  ...
  const getList = async () => {
    if (!universityId) {
      toast.error("Missing university context.");
      return;
    }
    ...
    try {
      ...
      // 1) Fetch only this university users
      ...
      // 2) Find expired users who are ONLY livingtype === "university"
      const expiredUniversityUsers = [];
      for (const u of users) {
        if (isProtected(u)) continue;

        const living = String(u.livingtype || "").trim().toLowerCase();
        if (living !== "university") continue;

        const end = String(u.studentVerifyEnd || "").trim().slice(0, 10);
        if (end && end < today) {
          expiredUniversityUsers.push(u.id);
        }
      }

      // 3) Batch update: university -> outside
      if (expiredUniversityUsers.length) {
        let batch = writeBatch(db);
        let count = 0;

        for (const uid of expiredUniversityUsers) {
      ...
      // 4) Update UI list locally
      const updatedList = users
        .map((u) => {
          if (isProtected(u)) return null;

          const living = String(u.livingtype || "").trim().toLowerCase();
          if (living !== "university") return u;
  ...
  const handleUploadExcel = async () => {
    ...
    try {
      ...
      // Fetch users in this university
      const usersQuery = query(
        collection(db, "users"),
        where("universityid", "==", universityId)
      );
      ...
      // Save date range and activate university livingtype
      try {
  ...
  const handleUniversityApproval = async (uid, action) => {
    try {
      const ok = window.confirm(
        `Are you sure you want to ${action.toUpperCase()} this user?`
      );
      if (!ok) return;
      ...
      if (action === "approved") {
        await updateDoc(userRef, {
          ...
          livingtype: "university",
          universityApprovalUpdatedAt: serverTimestamp(),
          universityApprovalUpdatedBy: myUid || null,
          universityApprovalReason: null,
        });
      } else {
        await updateDoc(userRef, {
          ...
          universityApprovalUpdatedAt: serverTimestamp(),
          universityApprovalUpdatedBy: myUid || null,
          universityApprovalReason: "Rejected by admin",
        });
      }
      ...
      console.error(e);
      toast.error("Failed to update university approval.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const filteredList = useMemo(() => {
    ...
    return list.filter((item) => {
      const name = String(item.username || item.name || "").toLowerCase();
      const email = String(item.email || "").toLowerCase();
      const address = String(item.address || "").toLowerCase();
      const studentId = String(
        item.studentid || item.studentID || item.sid || ""
      ).toLowerCase();
  ...
  const sortedList = useMemo(() => {
    return [...filteredList].sort((a, b) => getLatestMs(b) - getLatestMs(a));
  }, [filteredList]);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-3 justify-between items-center mb-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">University Students</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage student accounts for this university
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-3 bg-white border border-gray-300 px-4 py-2 rounded-xl">
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          placeholder="Search by name, email, address or student id..."
          className="w-full bg-white border border-gray-300 rounded-xl pl-10 pr-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-black"
        />
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {paginatedData.length === 0 ? (
                ...
                paginatedData.map((item) => {
                  const isPending =
                    String(item.universityApprovalStatus || "") === "pending";
                  const isDisabled =
                    String(item.accountStatus || "active") === "disabled";
                    ...
                    <tr key={item.id}>
                      ...
                      <td className="px-6 py-4">
                        {item.profileImage || item.photoURL ? (
                          <img
                            src={item.profileImage || item.photoURL}
                            alt="avatar"
                            className="w-12 h-12 rounded-full object-cover"
                          />
                        ) : (
                          <div className="w-12 h-12 rounded-full bg-gray-300 flex items-center justify-center text-sm text-white font-semibold">
                            {(item.username ||
                              item.name ||
                              item.firstname ||
                              "U")
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          {isPending && (
                            <>
                              <button
                                onClick={() =>
                                  handleUniversityApproval(item.id, "approved")
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleUniversityApproval(item.id, "rejected")
                                }
                                disabled={isLoading || isUpdating}
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="space-x-2">
          <button
            ...
            className="px-3 py-1.5 bg-gray-200 rounded-lg disabled:opacity-50"
          >
            Previous
          </button>
