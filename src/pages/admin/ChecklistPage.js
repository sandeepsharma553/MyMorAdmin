...
export default function ChecklistPage({ navbarHeight }) {
  ...
  const loadChecklists = async () => {
    ...
    try {
      const snap = await getDocs(
        query(collection(db, "hostel", hostelId, "checklists"), orderBy("rank", "asc"))
      );
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing?.id) {
        ...
        await addDoc(collection(db, "hostel", hostelId, "checklists"), {
          ...data,
  ...
  const togglePin = async (cl) => {
    try {
      await updateDoc(doc(db, "hostel", hostelId, "checklists", cl.id), {
        isPinned: !cl.isPinned,
      });
      loadChecklists();
    } catch (e) {
              ...
              <div key={cl.id} className="bg-white rounded shadow overflow-hidden">
                <div className="flex items-center p-4 gap-3">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-semibold ${
                      cl.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
                    }`}
                  >
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => togglePin(cl)} className="text-gray-600 hover:underline">
                      {cl.isPinned ? <PinOff size={16} /> : <Pin size={16} />}
                    </button>
                    <button onClick={() => toggleActive(cl)} className="text-gray-600 hover:underline">
                      {cl.isActive ? <XCircle size={16} /> : <CheckCircle size={16} />}
                    </button>
                    <button onClick={() => openEdit(cl)} className="text-blue-600 hover:underline">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => deleteChecklist(cl)} className="text-red-600 hover:underline">
                      <Trash2 size={16} />
                    </button>
                    <button
                ...
                {expandedId === cl.id && (
                  <div className="border-t border-gray-200 px-4 py-4 bg-gray-50">
                    <p className="text-xs font-medium text-gray-500 mb-3">Tasks</p>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-start justify-center overflow-y-auto py-8 z-50">
          <div className="bg-white w-full max-w-2xl mx-4 p-6 rounded-lg shadow-lg">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">{editing ? "Edit Checklist" : "Create Checklist"}</h2>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={addTask} className="text-sm text-blue-600 hover:underline inline-flex items-center gap-1">
                ...
                {tasks.map((t, idx) => (
                  <div key={idx} className="border border-gray-200 rounded p-3 bg-gray-50">
                    <div className="flex gap-2 mb-2">
                      ...
                      />
                      <button onClick={() => removeTask(idx)} className="text-red-600 hover:text-red-700">
                        <Trash2 size={14} />
                      </button>
                    </div>
                    ...
                    <div className="flex gap-4 flex-wrap">
                      ...
                      {["photo", "video", "file"].map((type) => (
                        <label key={type} className="flex items-center gap-1 text-xs text-gray-700">
                          <input
                            type="checkbox"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white w-full max-w-md mx-4 p-6 rounded-lg shadow-lg">
            ...
            <p className="text-sm text-gray-500 mb-3">
              {reviewSub.userName} · {reviewSub.taskTitle}
            </p>
