...
export default function OrientationPage({ navbarHeight }) {
  ...
  const loadModules = async () => {
    ...
    try {
      const snap = await getDocs(
        query(
          collection(db, "hostel", hostelId, "orientationModules"),
          orderBy("order", "asc")
        )
      );
  ...
  const handleSave = async (e) => {
    ...
    try {
      const moduleMedia = await buildMediaPayload(
        form,
        `hostel/${hostelId}/orientationModules/module`
      );
      ...
      const uploadedSlides = await Promise.all(
        form.slides.map(async (slide, index) => {
          ...
          const slideMedia = await buildMediaPayload(
            slide,
            `hostel/${hostelId}/orientationModules/slides/slide-${index + 1}`
          );
      ...
      if (editing) {
        await updateDoc(
          doc(db, "hostel", hostelId, "orientationModules", editing.id),
          payload
        );
  ...
  const toggleActive = async (m) => {
    try {
      await updateDoc(
        doc(db, "hostel", hostelId, "orientationModules", m.id),
        { isActive: !m.isActive }
      );
      loadModules();
    } catch (err) {
