...
export default function WellbeingPage({ navbarHeight }) {
  ...
  const loadCheckIns = async () => {
    setLoading(true);
    try {
      const snap = await getDocs(collection(db, "hostel", hostelId, "wellbeingCheckIns"));
  ...
  const loadResources = async () => {
    setLoading(true);
    try {
      const snap = await getDocs(query(collection(db, "hostel", hostelId, "wellbeingResources"), orderBy("order", "asc")));
  ...
  const loadSettings = async () => {
    try {
      const snap = await getDoc(doc(db, "hostel", hostelId, "wellbeingSettings", "config"));
  ...
  const saveResource = async (e) => {
    ...
    try {
      ...
      if (editingResource) {
        await updateDoc(doc(db, "hostel", hostelId, "wellbeingResources", editingResource.id), payload);
        toast.success("Resource updated");
      } else {
        await addDoc(collection(db, "hostel", hostelId, "wellbeingResources"), { ...payload, createdAt: serverTimestamp(), createdBy: user?.uid || "" });
  ...
  const deleteResource = async (id) => {
    ...
    try { await deleteDoc(doc(db, "hostel", hostelId, "wellbeingResources", id)); toast.success("Deleted"); loadResources(); }
  ...
  const togglePin = async (r) => {
    ...
    try { await updateDoc(doc(db, "hostel", hostelId, "wellbeingResources", r.id), { isPinned: !r.isPinned }); loadResources(); }
  ...
  const saveSettings = async (e) => {
    ...
    setSaving(true);
    try {
      await setDoc(doc(db, "hostel", hostelId, "wellbeingSettings", "config"), { ...settings, updatedAt: serverTimestamp() });
