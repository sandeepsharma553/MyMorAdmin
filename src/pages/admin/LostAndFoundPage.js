...
export default function LostAndFoundPage({ navbarHeight }) {
  ...
  const loadListings = async () => {
    setLoading(true);
    try {
      const snap = await getDocs(query(collection(db, "hostel", hostelId, "lostAndFound"), orderBy("createdAt", "desc")));
  ...
  const loadSettings = async () => {
    try {
      const snap = await getDoc(doc(db, "hostel", hostelId, "lostAndFoundSettings", "config"));
  ...
  const updateStatus = async (id, status) => {
    try {
      await updateDoc(doc(db, "hostel", hostelId, "lostAndFound", id), {
        status,
  ...
  const handlePost = async (e) => {
    ...
    setSaving(true);
    try {
      const path = `${hostelId}/lostAndFound/${Date.now()}_${postForm.imageFile.name}`;
      ...
      await addDoc(collection(db, "hostel", hostelId, "lostAndFound"), {
        postedBy: "staff",
  ...
  const saveSettings = async (e) => {
    ...
    try {
      await setDoc(doc(db, "hostel", hostelId, "lostAndFoundSettings", "config"), { ...settings, updatedAt: serverTimestamp() });
