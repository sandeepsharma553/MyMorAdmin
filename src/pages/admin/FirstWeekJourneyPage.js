...
export default function FirstWeekJourneyPage({ navbarHeight }) {
  ...
  const loadTemplates = async () => {
    ...
    try {
      const snap = await getDocs(
        query(collection(db, "hostel", hostelId, "journeyTemplates"), orderBy("createdAt", "desc"))
      );
  ...
  const loadAnalytics = async () => {
    ...
    try {
      const [tSnap, jSnap] = await Promise.all([
        getDocs(collection(db, "hostel", hostelId, "journeyTemplates")),
        getDocs(collection(db, "hostel", hostelId, "studentJourneys")),
      ]);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await updateDoc(doc(db, "hostel", hostelId, "journeyTemplates", editing.id), payload);
        toast.success("Template updated");
      } else {
        await addDoc(collection(db, "hostel", hostelId, "journeyTemplates"), {
          ...payload,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteDoc(doc(db, "hostel", hostelId, "journeyTemplates", id));
  ...
  const toggleActive = async (t) => {
    try {
      await updateDoc(doc(db, "hostel", hostelId, "journeyTemplates", t.id), { isActive: !t.isActive });
      loadTemplates();
