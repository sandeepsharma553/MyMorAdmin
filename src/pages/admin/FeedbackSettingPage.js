...
const FeedbackSettingPage = (props) => {
  ...
  const getReportList = async () => {
    ...
    const maintenanceCategoryQuery = query(
      hostelCol(emp.hostelid, "feedbackitems")
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!form.name) return;
      if (editingData) {
        const docRef = doc(hostelCol(emp.hostelid, "feedbackitems"),form.id);
        const docSnap = await getDoc(docRef);
        if (!docSnap.exists()) {
          toast.warning("data does not exist! Cannot update.");
          return;
        }
        await updateDoc(doc(hostelCol(emp.hostelid, "feedbackitems"),form.id), {
          uid,
          name: form.name,
        ...
        const q = query(hostelCol(emp.hostelid, "feedbackitems"), where("name", "==", form.name));
        const querySnapshot = await getDocs(q);
        if (!querySnapshot.empty) {
          toast.warn("Duplicate found! Not adding.");
          return;
        }
        await addDoc(hostelCol(emp.hostelid, "feedbackitems"), {
          uid,
          name: form.name,
          hostelid: emp.hostelid,
          createdBy: uid,
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(hostelCol(emp.hostelid, "feedbackitems"),form.id));
  ...
  return (
    <main className="flex-1 p-6 bg-gray-100 overflow-auto">
      ...
      <div className="flex flex-wrap gap-3 justify-between items-center mb-4">
        <h1 className="text-2xl font-semibold">Feedback Setting</h1>
